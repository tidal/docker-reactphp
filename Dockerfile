...
RUN cd /usr/local/src && http://skylink.dl.sourceforge.net/project/levent/libevent/libevent-2.0/libevent-2.0.22-stable.tar.gz && tar -xvzf libevent-2.0.22-stable.tar.gz && rm *.gz && cd libevent-2.0.22-stable && ./configure && make && make install 
